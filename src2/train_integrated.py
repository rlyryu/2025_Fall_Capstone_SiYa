...
def plot_acc_curve(history, save_prefix=None, show=False):
    ...
    ax.plot(epochs, history["train_acc"], marker='o', label="Train Acc")
    ax.plot(epochs, history["val_acc"], marker='o', label="Val Acc")
    ax.set_xlabel("Epoch")
...
def forward_bulk_early_fusion_chunkwise(model, batch, config, device):
    ...
    return logits, mil_attn, wsi_embed
...
def main():
    ...
    train_samples, val_samples = prepare_data_splits(
        CONFIG["root_dir"], CONFIG["seed"], CONFIG["is_bulk"]
    )
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        if val_acc > best_val_acc:
            ...
            plot_confusion_matrix(
                ...
                save_path=os.path.join(exp_dir, f"confusion_matrix_epoch_{epoch+1}.png") # confusion_matrix_img_epoch_3.png
            )
            
            print(f"✓ Saved best model to: {ckpt_path} (val_acc={val_acc:.2f}%)")

    # plot learning curves
    plot_acc_curve(history, save_prefix=os.path.join(exp_dir, "learning_curve"), show=False)
    plot_loss_curve(history, save_prefix=os.path.join(exp_dir, "learning_curve"), show=False)
